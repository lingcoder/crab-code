...
| Total tests | ~4927 | `cargo nextest run --workspace` |
...
- **L2a (Swarm)** is a topology choice, selected with `CRAB_TEAM_MODE=peer-to-peer` (also accepts `p2p` / `swarm`; anything unrecognised warns and falls back). It reaches `TeamRunner::set_mode`, and `Team::can_communicate` then enforces it on every routed message. The default, `leader-worker`, keeps teammates answering the main agent only.
...
- **The loop is closed in both directions.** A teammate runs in a spawned task and cannot reach the runner, so the markers its own tools emit travel back over a channel (`TeamHandles::markers_tx`) the same way results do. After every turn the runner scans that teammate's conversation and forwards what it finds. Without this a teammate's `SendMessage` would land in a conversation nobody reads, which is what made the Swarm topology unreachable in practice.
  - A teammate may message peers, subject to the mode. Messages it addresses to `main` are folded into the parent conversation as `<teammate-message from="...">`.
  - A teammate may **not** spawn teammates: `Agent` is stripped from every teammate registry (`TEAMMATE_DENIED_TOOLS`), because nothing in the roster or the job registry bounds the recursion depth. A spawn marker arriving from a teammate is logged and dropped.
  - Self-addressed messages are skipped rather than looping.
- Spawn markers may declare `capabilities`, which land on the roster entry and drive `Team::members_with_capability`.
