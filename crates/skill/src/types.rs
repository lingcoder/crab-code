...
//! Aligned with CCB `types/command.ts` (`PromptCommand` + `BundledSkillDefinition`).
...
/// into the binary (bundled). Each skill produces prompt content that is
...
pub struct Skill {
    ...
    // ─── Reference files ────────────────────────────────────────────
    /// Files bundled with the skill, extracted to disk on first invocation.
...
mod tests {
    ...
    #[test]
    fn skill_source_default_is_bundled() {
        assert_eq!(SkillSource::default(), SkillSource::Bundled);
    }
